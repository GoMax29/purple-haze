...
export default function ConsensusChart({
  data, title, unit, color, gradientId,
  yDomain = ['auto', 'auto'], modelLines, seriesKey,
}: ConsensusChartProps) {
  const chartData = useMemo(() => {
    return data.map((p) => {
      const row: Record<string, unknown> = {
        hour: p.hour, datetime: p.datetime,
        value: p.value, min: p.min, max: p.max, modelCount: p.modelCount,
      };
    ...
    <div className="space-y-1">
      ...
      <div className={`${hasModels ? 'h-52 sm:h-64' : 'h-44 sm:h-56'} w-full`}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 8, right: 4, left: -10, bottom: 5 }}>
            ...
            <XAxis dataKey="hour" type="number" scale="linear" domain={[0, lastHour > 0 ? lastHour : 168]} ticks={xTicks} tickFormatter={(h: number) => `J${Math.floor(h / 24) + 1}`} tick={{ fontSize: 9, fill: '#94a3b8' }} axisLine={{ stroke: '#334155' }} tickLine={{ stroke: '#334155' }} />
            <YAxis tick={{ fontSize: 9, fill: '#94a3b8' }} axisLine={{ stroke: '#334155' }} tickLine={{ stroke: '#334155' }} tickFormatter={(v) => `${v}${unit}`} width={36} domain={yDomain} />
            {hasModels ? (
              <Tooltip content={<ConsensusTooltip nwpModels={nwpModels} unit={unit} consensusColor={color} />} cursor={{ stroke: '#475569', strokeWidth: 1 }} />
            ) : (
              <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid #334155', borderRadius: '8px', fontSize: '10px' }} itemStyle={{ color: '#e2e8f0' }} labelStyle={{ color: '#94a3b8' }} labelFormatter={(h: number, payload) => { const dt = payload?.[0]?.payload?.datetime as string | undefined; return dt ? formatDatetime(dt) : `H+${h}`; }} formatter={(value: number, name: string) => { const labels: Record<string, string> = { value: 'Consensus', max: 'Max modèles', min: 'Min modèles' }; return [`${Number(value).toFixed(0)}${unit}`, labels[name] || name]; }} />
            )}
            ...
            <Line type="monotone" dataKey="value" stroke={color} strokeWidth={2.5} dot={false} activeDot={{ r: 3, fill: color, stroke: '#0f172a', strokeWidth: 2 }} isAnimationActive={false} connectNulls={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
      ...
      {hasModels && (
        <div className="flex flex-wrap gap-x-2.5 gap-y-0.5 px-1">
          ...
          {nwpModels.map((m) => (<LegendItem key={m.key} color={m.color} label={m.name} dim />))}
        </div>
      )}
    </div>
  );
...
function ConsensusTooltip({ active, payload, nwpModels, unit, consensusColor }: {
  active?: boolean;
  payload?: Array<{ dataKey: string; value: number; stroke?: string; color?: string; payload: Record<string, unknown> }>;
  nwpModels: ModelLineInfo[]; unit: string; consensusColor: string;
}) {
  ...
  const resolved = raw.map((item) => {
    let name = item.dataKey; let itemColor = item.stroke || item.color || '#94a3b8'; let isConsensus = false;
    if (item.dataKey === 'value') { name = 'Consensus'; itemColor = consensusColor; isConsensus = true; }
    else { const nwp = nwpModels.find((m) => m.key === item.dataKey); if (nwp) { name = nwp.name; itemColor = nwp.color; } }
  ...
  return (
    <div className="bg-slate-900/95 border border-slate-700 rounded-lg px-3 py-2 shadow-xl text-[10px] max-w-[220px]">
      ...
      {ordered.map((item, i) => {
        const prev = ordered[i - 1]; const next = ordered[i + 1];
        ...
        return (
          <div key={item.dataKey}>
            ...
            <div className={`flex justify-between gap-3 py-0.5 ${item.isConsensus ? 'font-semibold' : ''}`}>
              ...
              <span className="text-slate-200 font-mono whitespace-nowrap">{Number(item.value).toFixed(0)}{unit}</span>
            </div>
