...
/**
 * Traite les données de température avec pondération gaussienne
 * @param {number} lat - Latitude
 * @param {number} lon - Longitude
 * @returns {Promise<Array<{datetime: string, value: number}>>} Données de température traitées
 */
export async function traiterTemperature(lat, lon) {
  try {
    // 1. Charger la configuration
    const configPath = path.join(process.cwd(), "config", "temperature.json");
    ...
    // 3. Extraire les données de température pour chaque modèle configuré
    const result = [];
    ...
    for (let hourIndex = 0; hourIndex < timestamps.length; hourIndex++) {
      ...
      Object.entries(config.models).forEach(([modelKey, modelConfig]) => {
        ...
        // La clé réelle est "parameter_modelname" (ex: "temperature_2m_meteofrance_arome_france")
    ...
    throw new Error(
      `Erreur lors du traitement de la température: ${error.message}`
    );
  }
}
 ...
 */
export async function getTemperatureStats(lat, lon) {
  try {
    const config = JSON.parse(
      fs.readFileSync(
        path.join(process.cwd(), "config", "temperature.json"),
        "utf-8"
      )
    );
    const data = await traiterTemperature(lat, lon);
...
export default traiterTemperature;
